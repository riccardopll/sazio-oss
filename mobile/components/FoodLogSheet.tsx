...
export type FoodLogSheetRef = {
  present: (params?: FoodLogSheetParams) => void;
  dismiss: () => void;
};
...
export const FoodLogSheet = forwardRef<FoodLogSheetRef, FoodLogSheetProps>(
  function FoodLogSheet({ onRequestCreateFood }, ref) {
    const trpc = useTRPC();
    const queryClient = useQueryClient();
    const sheetRef = useRef<TrueSheet>(null);
    const [params, setParams] = useState<FoodLogSheetParams>({});
    ...
    useImperativeHandle(ref, () => ({
      present: (nextParams?: FoodLogSheetParams) => {
    ...
    useEffect(() => {
      ...
      setQuantity("1");
    }, [params.initialSearch, params.selectedFood]);
    ...
    const previewMacros = selectedFood
      ...
      : null;
    const previewCalories = previewMacros
      ? calculateCalories(previewMacros)
      : 0;
      ...
      <TrueSheet
        ...
        header={
          <View className="flex-row items-center justify-between border-b border-border-subtle px-4 py-4">
            ...
            <Pressable
              onPress={handleSave}
              disabled={createFoodLog.isPending || !selectedFood}
            >
              <Text
                className={`text-base font-semibold ${createFoodLog.isPending || !selectedFood ? "text-text-muted" : "text-text-primary"}`}
              >
                {createFoodLog.isPending ? "Saving..." : "Save"}
              </Text>
            </Pressable>
          </View>
        }
                ...
                {foodsQuery.isLoading ? (
                  ...
                  <View className="px-4 py-5">
                    ...
                    <Text className="mt-1 text-sm leading-5 text-text-secondary">
                      Create your first food and it will be ready to log right
                      away.
                    </Text>
                  </View>
                ) : (
